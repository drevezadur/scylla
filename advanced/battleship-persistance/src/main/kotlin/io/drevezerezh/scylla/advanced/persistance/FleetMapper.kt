...
object FleetMapper {
    ...
    fun toPersistance(domain: Fleet): FleetPJson {
        return FleetPJson(
            domain.id,
            domain.battleId,
            domain.player,
            domain.status,
            GridLocation.toText(domain.shots)
        )
    }


    fun toDomain(persisted: FleetPJson): Fleet {
        return Fleet(
            persisted.battleId,
            persisted.player,
            persisted.status,
            GridLocation.toList(persisted.shots).toSet()
        )
    }
}