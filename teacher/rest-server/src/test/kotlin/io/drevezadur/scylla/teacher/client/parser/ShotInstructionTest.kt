...
class ShotInstructionTest {
    ...
    @BeforeEach
    fun setUp() {
        instruction = ShotInstruction(TARGET)
    }

    @Test
    fun `Shall call shot of interpreter`() {
        every { interpreter.shot(TARGET) } answers { nothing }

        instruction.execute(interpreter)

        verify(exactly = 1) { interpreter.shot(TARGET) }
    }

    companion object {
        val TARGET = Location(3, 7)
    }
}