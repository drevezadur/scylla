...
@Service
class ShotUseCaseManagerImpl(
    @Autowired
    private val shotResolverFactory: ShotResolverFactory
) : ShotUseCaseManager {

    override fun shotAt(battleId: BattleUUID, fromPlayerId: PlayerUUID, target: Location): ShotReport {
        val shotResolver = shotResolverFactory.createInstance()
        return shotResolver.resolve(battleId, fromPlayerId, target)
    }
}